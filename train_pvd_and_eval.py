...
def generate_eval(model, opt, gpu, outf_syn, evaluator):
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm(enumerate(test_dataloader), total=len(test_dataloader), desc='Generating Samples'):
            ...
            x = data['train_points'].transpose(1,2) # [B, N, 3]
            ...
            # gen -= gen.mean(dim=1, keepdim=True)
            # gen /= gen.norm(dim=2, keepdim=True).max(dim=1, keepdim=True)[0]
            # x -= x.mean(dim=1, keepdim=True)
            # x /= x.norm(dim=2, keepdim=True).max(dim=1, keepdim=True)[0]
            
            # bounding box normalization
            # gen -= gen.min(dim=1, keepdim=True)[0]
            # gen /= gen.max(dim=1, keepdim=True)[0]
            ...
            # save_image(gen, os.path.join(ep_save_dir,'samples_eval.png'), nrow=5)
    ...
    # m, s = test_dataset.all_points_mean, test_dataset.all_points_std
    # m, s = torch.tensor(m).float().to(gpu), torch.tensor(s).float().to(gpu)
    # samples = samples * s + m
    # reference = reference * s + m
    
    results = compute_all_metrics(samples, reference, opt.bs)
    # results = compute_all_metrics_mp(samples_gather, reference_gather, opt.bs)
    # results = compute_all_metrics_mp(samples, reference, opt.bs)
    results = {k: (v.cpu().detach().item()
                if not isinstance(v, float) else v) for k, v in results.items()}
    ...
    '''
    #  model name _ n layers _ n hidden _ patch size _ n heads
    # stats['model_name'] = opt.model_type
    # stats['Epoch'] = opt.niter-1
    # stats['n_params'] = sum(param.numel() for param in model.parameters())/1e6
    # stats['n_layers'] = model.model.depth
    # stats['n_hidden'] = model.model.hidden_size
    # stats['patch_size'] = int(opt.n_p**0.5)
    # stats['n_heads'] = model.model.num_heads
    return stats
...
def test(gpu, opt, output_dir):
    ...
    with torch.no_grad():
        ...
        if opt.distribution_type == 'multi':
            ...
            resumed_param = torch.load(opt.model)
            resumed_param = {k.replace('module.', ''): v for k, v in resumed_param['model_state'].items()}
            model.load_state_dict(resumed_param)

        opt.eval_path = os.path.join(outf_syn, 'samples.pth')
        Path(opt.eval_path).parent.mkdir(parents=True, exist_ok=True)
        
        for _ in range(opt.nrepeats):
...
def train(gpu, opt, output_dir, noises_init):
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        if (epoch + 1) % opt.saveIter == 0 or epoch == opt.niter - 1:
...
def main():
    ...
    train_dataset, _ = get_dataset(opt.dataroot, opt.npoints, opt.category)
    if opt.fps_points > 0:
        noises_init = torch.randn(len(train_dataset), opt.fps_points, opt.nc)
    else:
...
def parse_args():
    ...
    parser.add_argument('--experiment_name', type=str, default='pvd', help='experiment name (used for checkpointing and logging)')
    # Data params
    parser.add_argument('--dataroot', required=True, help='path to PointFlow-Data/ShapeNetCore.v2.PC15k')
    ...
    parser.add_argument('--npoints', default=8192)
    parser.add_argument('--fps_points', default=2048)
    '''model'''

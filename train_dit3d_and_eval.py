...
def train(gpu, opt, output_dir, noises_init):
    ...
    if opt.model != '':
        if opt.use_ema:
            ...
            checkpoint_dict = {k.replace('model.', 'model.module.'): ema_weights[k] for k in ema_weights if k.startswith('model.')}
            model.load_state_dict(checkpoint_dict)
        else:
            model.load_state_dict(torch.load(opt.model)['model_state'])
...
def generate_eval(model, opt, gpu, outf_syn, evaluator):
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm(enumerate(test_dataloader), total=len(test_dataloader), desc='Generating Samples'):
            ...
            # print(gen.shape, x.shape, s.shape, m.shape)
            gen = gen * s + m
            x = x * s + m
    ...
    logger.info('Samples shape: %s' % str(samples_gather.shape))
    logger.info('Reference shape: %s' % str(reference_gather.shape))
    
    results = compute_all_metrics(samples_gather, reference_gather, opt.bs)
    results = {k: (v.cpu().detach().item()
                if not isinstance(v, float) else v) for k, v in results.items()}
...
def parse_args():
    ...
    parser.add_argument('--vizIter', default=2000, type=int, help='unit: epoch')
